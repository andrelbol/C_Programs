...
task_queue_t * get_shortest_time_task(task_queue_t ** queue){
  /* Return the highest priority task*/
  task_queue_t * shortest_time_task = *queue;
  task_queue_t * aux = shortest_time_task->next;
  while(aux != *queue){
    if(aux->all_duration < shortest_time_task->all_duration){
      shortest_time_task = aux;
    }
    aux = aux->next;
  }
  return shortest_time_task;
}

int main(int argc, char const *argv[]){
  task_queue_t *iterator; // User for iterating in queue
  task_queue_t *aux; // User for iterating in queue
  task_queue_t *shortest_time_task; // Contains the task with highest priority in ready queue
  int i=0, size;
  ...
  while(t < TMAX){
    ...
    if(cpu_state == FREE){
      if(ready_tasks != NULL){
        shortest_time_task = get_shortest_time_task(&ready_tasks); // Get the highest priority task
        current_task = (task_queue_t *) queue_remove((queue_t **) &ready_tasks, (queue_t *) shortest_time_task);
