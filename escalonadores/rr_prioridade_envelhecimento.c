...
task_queue_t * get_high_priority_task(task_queue_t ** queue){
  ...
  while(aux != *queue){
    if((aux->static_priority + aux->dynamic_priority) > (high_priority_task->static_priority + high_priority_task->dynamic_priority)){
    ...
    if(cpu_state == BUSY){ // If there is a task in
      if(current_task->current_duration == current_task->all_duration){
        ...
        if(current_task->cpu_duration == current_task->quantum){ // End of quantum time
          ...
          current_task->dynamic_priority++; // Aging process
          cpu_state = FREE;
