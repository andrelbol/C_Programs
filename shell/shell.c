 ...
 * comando, o cÃ³digo converte um *cmd para o tipo especÃ­fico de
 * comando. */
struct cmd {
  int type; /* ' ' (exec)
               '|' (pipe)
...
struct redircmd {
  ...
  char *file;        // o arquivo de entrada ou saí­da
...
runcmd(struct cmd *cmd)
{
  ...
  case '<':
    ...
    close(rcmd->fd); // Closing file descriptor so the open gets the stdout
  ...
  case '|':
    ...
    pipe(p);
    if(fork() == 0){ // Child process is gonna read on pipe
      dup2(p[1], 1);
      close(p[0]);
      close(0);
      runcmd(pcmd->left);
    } else {  // Father process is gonna write on pipe 
      dup2(p[0], 0);
      close(p[1]);
      runcmd(pcmd->right);
    }
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    /* MARK START task1 */
    /* TAREFA1: O que faz o if abaixo e por que ele Ã© necessÃ¡rio?
     * Insira sua resposta no cÃ³digo e modifique o fprintf abaixo
     * para reportar o erro corretamente. */
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "O parâmetro passado não é um diretório ou você não tem permissão para acessá-lo.\n");
      continue;
    }
    /* MARK END task1 */
